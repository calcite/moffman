...
<#
.Description
Make file for Multi-Office Manager
#>
...
#>
function lint() {
    poetry run flake8 src\moffman tests
}
...
#>
function test() {
    poetry run python test.test_moffman.py
}
...
#>
function coverage() {
    poetry run coverage run --source src\moffman -m test.test_moffman.py
...
#>
function docs(){
    if (Test-Path ".\docs\moffman.rst") { Remove-Item ".\docs\moffman.rst" -Force }
    if (Test-Path ".\docs\modules.rst") { Remove-Item ".\docs\modules.rst" -Force }
    poetry run sphinx-apidoc -o docs\ src\moffman
